...
function ViewTeleDetails() {
  ...
  useEffect(() => {
    ...
    const fetchTeleconsultations = async () => {
      try {
        const response = await fetch(`http://localhost:5000/doctor/teleconsultations-view/${doctor_id}`);
        if (response.ok) {
  ...
  return (
    <div className="container mx-auto p-4 min-h-[70vh]">
      <h1 className="text-2xl font-semibold mb-4">Today Teleconsultations</h1>
      {loading ? (
...
export default ViewTeleDetails;
