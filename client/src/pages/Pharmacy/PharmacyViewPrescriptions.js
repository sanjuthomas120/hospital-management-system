...
function PharmacyViewPrescriptions() {
  ...
  const handleDetails = (prescriptionId) => {
    navigate(`/pharmacy/view-prescription-details/${prescriptionId}`);
  };
      ...
      <table className="min-w-full bg-white border">
        ...
        <tbody>
          {filteredPrescriptions.map((prescription) => {
            if (prescription.medicines && prescription.medicines.length > 0 && prescription.pharmacyConfirm === 'not confirmed') {
              return (
...
export default PharmacyViewPrescriptions;
